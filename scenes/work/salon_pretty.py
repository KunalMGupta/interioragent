...
Layout strategy (drives a WIDE, shallow room — salons are wide): load the two LONG walls and
keep the two SHORT walls light.
  - back (long)  : the styling row — 5 MirrorStationGroup units (chair + console + mirror + trolley).
  - front (long) : a prominent reception (desk + chair + decor) and the waiting nook (with greenery),
                   under a beauty-salon portrait + neon sign.
  - left/right (short): the "cabinets" — retail shelf (front end, by reception) on one short wall,
                   backwash row (center/front, away from the styling row) on the other.
  - openings (door/window) tuck onto the short walls.
"""
from IDSDL.scene import SceneProgRoom

scene = SceneProgRoom("HairSalonPretty", seed=77)

# --- back (long) wall: 5 styling stations (chair + console + mirror + side trolley) ---
def styling_station():
...
# --- the "cabinets" for the short walls ---
backwash = 2 * scene.AddAsset("a salon backwash shampoo unit")

# --- front (long) wall cluster 1: a prominent reception — big desk + chair behind + decor ---
...
# --- front (long) wall cluster 2: waiting nook — blush velvet pair + brass table + rug ---
...
with scene.RoomGroup(modulate_scale=0.92, randomness=0.12) as room:
    ...
    # back (long): styling stations FLUSH against the wall (mirrors sit on it, not floating).
    # facing="back" both orients the spine (rotation 180) AND tells the auto-sizer how deep the
    # back wall row must be — so no redundant room.face() is needed.
    room.place_on_back_wall_center(spine, facing="back")
    # front (long): reception + waiting clusters across the opposing wide wall, facing into the room
    room.place_on_front_left(reception, facing="back")
    room.place_on_front(waiting)
    # left/right (short): the cabinets. Retail shelf shifted to the front (reception) end of the
    # left wall; backwash moved to the center/front of the right wall, away from the styling row.
    ...
    # decor + sign on the front wall: a beauty-salon portrait over the reception, neon at center
