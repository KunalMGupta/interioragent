...
with scene.RoomGroup(modulate_scale=1.0, randomness=0.08) as room:
    ...
    if PHASE >= 3:
        ...
        hood.scale(1.6)                     # uniform: width ~ the range module below it
        hood.ignore_overlap = True
        hood.is_static = True
        room.place_on_back_wall_center(hood, bottom=1.55)
        # the hanging utensil rail over the wash counter (slot verb = art-band height; if the
        # wall scaler re-derives it deeper than 0.25 m it will WARN and it comes off — museum rule)
        room.place_on_wall_left_center(
            scene.AddAsset("a stainless steel kitchen utensil set hanging on a rail", asset_id=UTENSIL))
