...
with scene.RelativeGroup() as seating:
    ...
    if PHASE >= 2:
        ...
        # 2026-07-14, up from 0.75) pulls the sectional + nook onto one rug to read as a single
        # cohesive conversation zone (Kunal: "more cohesive to appear cozy") while still letting
        # a margin of dark walnut show around the edge
    if PHASE >= 3:
...
with scene.RoomGroup(modulate_scale=0.85, randomness=0.15) as room:
    ...
    # had voted enlarge every phase (1.2/1.25/1.1/1.1) and the build shipped at 1.1 — but that
    # vote optimizes for UNCLUTTERED, and "cozy" is the opposite brief: the walls should press in
    # so the single seating cluster fills the room. This is a DELIBERATE human override of the
    # enlarge vote (it will vote enlarge again on rebuild — refuse it; the brief wins). 0.85 pulls
    # the shell ~23% tighter than the shipped 1.1 without crowding the hearth clearance.
