...
A cheerful children's toy shop. Defining moves (from the plan + collage):
  * PERIMETER MERCHANDISING — PRE-STOCKED shop fixtures (cartoon toy shelves, a colourful comic-book
    wall, book/game shelves, a figurine tower) ring the back and side walls, a few crowned with a
    hero toy (dollhouse / teddy / giraffe / blocks). This is the florist "mass ONE abundant prop"
    identity: colourful kids' merchandise everywhere carries the room.
  * CENTRAL PLAY ISLAND (hero) — a low round kids' table on a round multicolour rug, a wooden
    train + building blocks on top, flanked by a rocking horse and a ride-on car.
  * A TEEPEE READING NOOK in the front-left corner (kids' teepee + book display + big teddy + cushion).
  * A NEAR-ENTRANCE CHECKOUT (counter + POS + bag) in the front-right, by the storefront.
  * Colourful display tables massing boxed toys / board games to fill the floor.

Palette (plan): white walls, warm wood/laminate floor, bright primary-colour toys as the accent.

Recipe notes / iteration log:
  * Opened with a retrieval STRESS TEST (40/40 solid, 0 gaps) — strong-coverage category, no ingest.
  * Perimeter shelves are RelativeGroups (anchor = shelf, toy `place_on_top`) so one wall call
    seats shelf + toy; place_on_*_wall_* clamps to wall span + ceiling.
  * `add_lighting` must be a FLAT/FLUSH fixture (a hanging rig blows out — see executive_office).
  * Iter 1 -> 2: (a) fixed a core `object.scale()` bug (was blowing up non-unit-scale assets like the
    rocking horse to ~3 m); scales below are now literal metres. (b) dropped the 3.84 m TALL_SHELF
    (auto-clamped into thin EMPTY verticals) for colourful book-displays + properly-sized gondolas.
    (c) crowned shelves with bigger hero toys; (d) tightened the room + added a 3rd display table.
  * Iter 3: rebuilt the perimeter on the new ShopFixtureRetriever — the old white gondola read as a
    SHOE rack. Swapped in PRE-STOCKED shop fixtures (cartoon toy shelves, a colourful comic-book wall
    w/ red base, cartoon book/game shelves, a figurine tower) so the shelves themselves carry the
    "packed with toys" identity. Avoided the D=1.9 m multi-part comic wall (juts into the room).
"""
...
def display_table(tops):
    """A low kids' table massing colourful boxed toys / games / plush on top."""
...
table_games = display_table([sized("a colourful board game box", BOARDGAME, 0.32),
                             scene.AddAsset("a colourful jigsaw puzzle box"),
                             sized("a set of natural wooden building blocks", BLOCKS, 0.28)])
table_plush = display_table([sized("a plush giraffe stuffed animal toy", GIRAFFE, 0.28),
                             scene.AddAsset("a small colourful plush toy")])
table_cars  = display_table([sized("a colourful board game box", BOARDGAME, 0.3),
...
with scene.RoomGroup(modulate_scale=0.98, randomness=0.1) as room:   # tightened (VLM wanted less floor)
    ...
    # Brand focal wall sign + openings + light.
    ...
    room.place_door("right_wall", position="right")               # entrance, near checkout

    room.rotate(pos, 180)   # keep the checkout screen facing the approaching customer

scene.export("toy_shop.blend")
