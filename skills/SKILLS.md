...
3. **[examples/](examples/)** — the closest matching scene type. **Start at [examples/README.md](examples/README.md)** — the one-line-per-recipe human index. Every example carries frontmatter (`family` / `pattern` / `read_for`), and the catalog groups recipes by layout FAMILY (set-piece-hero, hero-anchor-room, table-hub, repeated-unit-grid, rows-runs-corridors, zoned-multi-zone, service-spine-counter, retail-spine-loop, subtractive). Copy the closest skeleton, don't start from scratch — each example's own `.md` carries its lessons, and the catalog links the atomic lessons minted on that scene. Four canonical references:
   - dental_office — a **single-room build hung on one ingested "unit/set" hero asset**;
   - executive_office — a **single room split into work + lounge zones around a storage backbone**; carries the `add_lighting` "flush fixture, never a chandelier" lesson;
   - retail_store — a **shop/showroom: a central double-sided-rail spine + perimeter merch loop + branded service wall**; carries the "lighting density scales with floor area" + "storefront = worst-case black void" lessons;
   - jewelry_shop — **making a retail scene read as its category by showing the PRODUCT at viewing height** (mass jewelry props on a display table + counter + pedestals — empty glass display cabinets read as furniture, not a shop); carries the "VLM loop verifies geometry, not category legibility — human gut-check a retail scene" + "reword counter→display cabinet" + "pin anything whose colour carries the palette" lessons.
...
6. **[workflow/constraints.md](workflow/constraints.md)** and **[workflow/vlm_feedback.md](workflow/vlm_feedback.md)** — keep open while optimizing. **[workflow/constraint_playbook.md](workflow/constraint_playbook.md)** is the per-asset WHICH-constraint catalog (FROZEN 2026-07-14).
...
8b. **[acquire-assets/SKILL.md](acquire-assets/SKILL.md)** — when the asset **does not exist in the library at all**. `python -m IDSDL.shop run "<query>"` searches Sketchfab, normalizes in Blender (single mesh, real metres, front=+Z), triages with a VLM, verifies the front on a re-render, and ingests — asking you (`shops/<batch>/HELP.md`) about anything it will not guess at. `--source meshy` GENERATES the asset instead. Use it *instead of* forcing a bad retrieval: the stress test in `workflow/asset_selection.md` tells you there is a gap; this fills it.
