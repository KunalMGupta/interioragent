...
| `GridGroup(randomness=)` | row/grid gaps; on `place_arc` the seat gap and row gap (a wide-and-shallow seating bank: front row shortest, gently bowed rows on one shared linear pitch, brick-staggered half a pitch, no seat more than ~40° off the bank axis) | jitters inter-item gaps (works even at `sparsity=0`) + turns each item up to ±14° off its row/arc facing (tamer than free groups so the formation survives; draws floored at 40% of max) |
